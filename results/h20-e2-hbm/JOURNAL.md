...
- Noise decision: stable if paired coTensor-minus-native deltas have the same
  nonzero page-sized mode in at least 4/5 ABBA blocks; otherwise classify as
  launch noise with raw rows retained.
- Correctness PASS: native/coTensor output hashes match, no fallback, no
  lifecycle error, and post-row GPU/FD state returns to baseline.
- Strict E2 PASS: coTensor steady HBM <= paired native steady HBM, median TTFT
  and prefill regression <=3%, with full E1/lifecycle tests passing.
...
- Acceptance uses DP-rank-stratified steady medians: served DP0 is
  ...
  both strata. The first native cold row `[80,648, 81,098]` is retained but
  excluded from steady parity. Unstratified median row maxima are native
...
- Physical ownership is equal to native. Per rank, coTensor has 48 local
